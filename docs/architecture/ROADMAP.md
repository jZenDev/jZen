...
| **In-app registration is broken against a confirm-required Supabase** — signup returns a user with no session when email confirmation is on, and the register flow expects an immediate session (true only under the local autoconfirm stack). | Registering via the deployed web app returned 401 "Supabase rejected"; a direct GoTrue signup returned 200. | **Open** — fix is the proper email-confirmation flow (register returns "check your email"; a confirmation callback sets the session). |
