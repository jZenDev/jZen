...
- **Verified:** `server/` builds all backend modules green (`./mvnw package`); the Dart
  ...
  parses and `task doctor` reports the toolchain; the `verify:contracts` gate catches a
...
|---|---|---|
| **The deployed web app shipped a bundle calling `http://localhost:18080`** — Flutter's web build cache does not invalidate on a `--dart-define` change, so every deploy after `test:native` reused a bundle built for its smoke container. The page loaded perfectly and its first API call was refused by the CSP for naming a foreign origin. | Trying to log in on the deployed app. Nothing else could see it: `build:web` printed the right URL because it prints what it *requested*, `flutter` succeeded, the deploy succeeded, and `verify:deploy` passed — every check it makes is unauthenticated. Revision `00016` shipped it too. | **Fixed (ADR-037).** `build:web` clears the build output before compiling and then reads the staged bundle for the host it was told to use, failing if absent. Byte search, not `strings`, which finds nothing in a dart2wasm bundle and would pass vacuously. Verified on the **served** bundle and by the page calling `/api/v1/auth/identity` same-origin (`204`). |
| **Three tables were published to the internet by the Supabase Data API** — `zen_jobs`, `zen_rate_limit_counters` and `flyway_schema_history` had no row-level security, and PostgREST serves the `public` schema to anyone holding the anon key. Not a leak of secrets: a leak of *control*, since one `UPDATE` switches off the retention job that discharges the GDPR obligation and a `DELETE` clears the durable rate-limit windows, both silently. | Reviewing the ROADMAP to decide whether the environment could be torn down. The Supabase dashboard advisor flagged the tables; a `curl` against the deployed project with the anon key then returned **200 with every row**, and write probes returned **204**. `users` answered `200 []` — its policy working, which is what made the other three legible. | **Fixed (ADR-036).** Two independent layers: RLS plus a `zen_runtime` application policy on each table, shipped by the module that owns it, and a schema-wide revoke of `anon`/`authenticated` that also covers tables a later migration creates. STANDARDS "Database migrations" gains the rule whose absence caused it — a new table ships RLS and a policy in the same change. The gap was not carelessness: it was two modules each adding a table the ordinary way, with no rule connecting them to ADR-031. |
...
| 4 | **Deploy the Flutter web app, as WebAssembly** | Same-origin, per the constraint above. The POC's user-facing surface. Built with `--wasm` (dart2wasm + skwasm) from the outset — **not** dart2js first and Wasm later: proving one compilation and shipping another is the native-vs-JVM trap that put a reflection 500 in production (see the native-JSON entry). `test:native` asserts the real Wasm bundle (`main.dart.wasm` served as `application/wasm`) in Docker before deploy. [`DECISIONS.md`](./DECISIONS.md) ADR-016 has the decision and the WasmGC browser floor it accepts. Blocked by 3. |
| 5 | **Deploy the admin panel** | Same container, same origin. Part of the POC, not after it — see below. |
| — | *POC complete* | Backend, web app (as Wasm), and admin panel, all deployed same-origin: all three language bindings of the contract-first claim demonstrated on live infrastructure. |
| 6 | **CI** | Sustains quality; it does not produce the first deploy. **Done, and kept current:** `.github/workflows/ci.yml` runs the same `task` targets a developer runs — `verify:boundaries`, `verify:contracts`, `verify:docs`, `test:client`, `test:admin`, `build:web`, `build:apps:runners` (Android + Linux desktop on `ubuntu-latest`, the Windows app on `windows-latest`; Apple targets need a macOS runner and are skipped *audibly*, being the ones a developer's own machine can still verify — ADR-045), `test:apps:server`, `test:e2e`. A gate that is written but not wired runs only when someone remembers, which is how a broken web bundle reached a deploy once. |
...
would defeat `verify:contracts`; that is now a STANDARDS "Orchestration" rule, since the hazard is
...
|---|---|---|---|
| 1 | **Publish the packages** to real registries (pub.dev, a Maven registry, npm) | **Postponed, and now with a trigger: after the second product exists** (ADR-020). Not idleness — while the framework has one consumer written by its own authors, an awkward API is fixed in a single commit and never felt, so `zen_demo` cannot falsify the framework/application boundary. A second, different product can. Publishing before that freezes an API which is about to be bent, and it is the one irreversible step here: a published version is retractable, never removable. **Trigger sharpened by ADR-026, which found this half wrong:** path dependencies *do* reach across repositories in Dart, and TypeScript needs no dependency edge at all. Only **Java** cannot — and even Java is satisfied locally by a root Maven aggregator over both checkouts. So what forces publishing is not a separate repository but **the second product's first CI run**: a lone clone has no sibling `jzen/` and no aggregator, so the Java half must then resolve from a registry. | ADR-020; ADR-026; "what is not yet production-ready" gap table |
...
| 7 | **`task` `sources:`/`generates:` fingerprinting** | **Refused, not open** — it would defeat `verify:contracts`; now a STANDARDS "Orchestration" rule. Listed for completeness. | STANDARDS "Orchestration"; ADR-014 |
| 8 | **Custom-scheme hijacking: move the email link to App Links / Universal Links** | **Android: done (ADR-025). iOS: server ready, app half needs a paid Apple membership.** It is a real vulnerability, not hardening. Any app on the device may register `zendemo://`, and an email link carries a live access **and** refresh token in its fragment — so an attacker who wins the scheme receives a working session for someone else's account, from an entirely genuine email the victim was right to trust. The server's exact-match allowlist (ADR-019) does not help: the attacker uses the *permitted* scheme. The Android half was never actually blocked on signing: an App Link verifies against the SHA-256 of whatever key signed the build, and a debug keystore is a fine key to list — treating the item as indivisible is what kept it closed. iOS still needs the Associated Domains capability, which only a paid membership can sign — and **this project's Apple account is a free personal team, so iOS cannot adopt App Links at all today**. That is an account-tier fact, not an amount of work: iOS stays on the custom scheme and therefore stays exposed to scheme hijacking until a paid membership exists. Worth naming plainly rather than leaving as "pending", because nothing in the codebase will change it. | **"Item 8 in full" below**; ADR-025; RFC 8252 §8.6 |
...
3. **Deliver the URL to `consumeAuthLink`.** A deep-link plugin (declared as a normal dependency —
   ...
   - *warm* — the app is running and the OS hands it a URL. This is what `consumeAuthLink` is for.
     Its **outcome has to be shown**, and that is easy to miss: a refused warm link deliberately
     changes no state (it must not sign out whoever is using the app now), so no screen has
     anything to react to and the tap looks ignored. `ZenAuthLinkListener`, placed inside
     `MaterialApp`, is the framework's answer — the receiver sits *above* `MaterialApp` to outlive
     every screen, the reporter *below* it to reach the `ScaffoldMessenger`.
