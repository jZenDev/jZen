...
import dev.zen.identity.AuthException;
import dev.zen.identity.IdentityService;
import dev.zen.identity.user.User;
import dev.zen.proto.v1.DemoProfile;
 ...
 *
 * <p>Same rules as {@link dev.zen.app.health.HealthResource}: every method returns
 * {@link Response} wrapping a proto message (a bare proto return type triggers Quarkus's
 * build-time Jackson writer and 500s, TA-1) and declares its OpenAPI schema by {@code $ref}
 * into {@code META-INF/openapi.yaml}. The {@code dev.zen.transport} seam picks JSON or Protobuf
 * from {@code X-Zen-Transport}; the method never names a wire format. This is what lets the demo
 * prove a typed round-trip in both transport modes.
 */
@Path("/api/v1/demo")
public class DemoResource {
  ...
  public Response ping(@HeaderParam(HttpHeaders.ACCEPT_LANGUAGE) String acceptLanguage) {
    String locale = messages.resolveLocale(acceptLanguage);
    Ping ping =
        Ping.newBuilder()
            .setMessage(messages.get("ping.message", locale))
  ...
  @APIResponse(responseCode = "401", description = "No active session (ZenError)")
